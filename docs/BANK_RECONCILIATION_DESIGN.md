...
1. Enter receipts into SimplifAI as they arrive.  
2. Later upload the **bank Excel** as **ground truth**.  
3. Confirm every app transaction in the period appears in the bank (and every bank line is accounted for).  
4. Reach a **verified** state where bank balance and system net reconcile (difference = 0 after accounting for opening balance).  
5. Avoid double-counting **credit card** activity when both the CC statement and the bank settlement exist.
...
| 2 | Adds the expense/deposit in **Transactions** (manual or file) | Keep as primary entry path |
| 3 | At period end (e.g. weekly) downloads bank Excel | Dedicated **bank reconcile** upload |
| 4 | Checks app ↔ bank for that period both ways | Match / mark verified; surface leftovers |
| 5 | Confirms totals line up | Show **Bank balance vs system net** (target gap = 0) |
...
> **Bank statement balance − (Opening bank amount + System net for relevant period/scope) = 0**  
...
| Credit cards | CC Excel creates BUFFER expenses; bank later shows settlement | Same money can count twice in net |
...
8. **Credit card** lines and bank CC settlements do not double-count.  
9. Unmatched bank rows and unmatched app rows remain visible until resolved — and raise **high-priority alerts** automatically.  
10. Keep manual receipt entry as the daily habit; bank work is periodic.
...
| **Match** | Link between one bank line and one (or more — TBD) app transaction(s) |
| **Bank-verified** | App transaction matched to bank; stores verification metadata including bank **אסמכתא** (and still keeps our transaction ref) |
| **Unmatched bank** | Bank line with no app counterpart — needs entry or explanation |
| **Unmatched app** | App transaction in period with no bank counterpart — needs attention |
...
Go-live (production)
  ...
  Mark all existing txs (≤ D₀) Verified
  Dashboard: Last verification date = D₀

Daily (after D₀)
  Receipt → Add expense/deposit in Transactions
            (status: Unverified)

Periodic bank reconcile (e.g. weekly)
  Download bank Excel covering activity since last verification
  → Open “Bank reconcile”
  → System reads bank lines + balance; matches against Unverified (and period) app txs
  → She reviews:
        • Auto-matched → confirm → Verified + bank אסמכתא
        • Unmatched bank → add missing tx OR explain/ignore
        • Unmatched app → fix or explain
  → Screen shows Gap = Bank balance − (Opening + Net)
  → If leftovers or Gap ≠ 0:
        • High-priority alerts open automatically (see §13a)
  → If session completes clean (Gap ≈ 0, no open unmatched):
        • Last verification date → statement end / D_new
        • Dashboard updates
```
...
**Important:** she works **verification → verification**. Between dates, Unverified rows and open reconcile alerts are the work queue; after a clean pass, the Dashboard date moves forward.
...
- **B** = bank balance from the statement (latest row’s balance column for the upload, or balance on period end date — **choose one**).  
- **O** = Opening bank amount (user-entered), as of date **D₀**.  
- **N** = System **company-float net** of transactions that **count toward bank reconciliation** with `transaction_date > D₀` and `≤` statement end (and optionally only **bank-verified** ones — see open questions).
...
Without **O**, a correct books state still shows a large Gap equal to the pre-SimplifAI bank balance (e.g. historical ~111k-style figures discussed earlier). Opening balance makes “Gap → 0” achievable on day one.

### Open product choices (need confirmation)

1. Does **N** include only **verified** rows, or all company-float rows in the period?  
   - *Recommendation:* show both — “verified net” vs “all net” — but **success criteria** use verified (or “verified + explicitly excluded”).  
2. Is **B** the last balance in the file, or balance on a chosen date?  
3. Is reconcile scoped to **company bank account only** (MIP Leumi ops), not per-property float?  
   - *Recommendation:* company operating account is the reconcile target; property float remains a separate management view.
...
Add a clear **Bank verified** state on each deposit/expense that participates in bank reconcile:

- **Unverified** (default for new manual/PDF/ledger entries that should hit the bank)  
- **Verified** (matched to a bank line, or manually marked after review)  
- **Excluded from bank reconcile** (optional): e.g. rental-only, owner-paid, or non-bank cash — so they don’t pollute unmatched lists  
...
- Prefer unverified app txs first  

One bank line ↔ one app transaction in v1 (split/merge later if needed).
...
## 11. Credit card double-count

### Problem

1. CC Excel (or manual) books merchant charges as expenses.  
2. Later, bank shows a debit for the **card payment / settlement**.  
3. If both count in company-float net, net is wrong vs bank.

### Design options (pick one primary for v1)

**Option A — Exclude bank CC settlements from net**  
- Detect bank lines that are credit-card payments (rules/keywords / linked CC account).  
- Those bank lines are used for **verification of settlement only**, or marked **Excluded from net**, while merchant-level CC expenses remain the economic expense.  
- Bank Gap equation uses bank balance (which already reflects the settlement) and system net **without** double expense.

**Option B — Exclude CC merchant expenses from company-float net; bank settlement counts**  
- Closer to “cash basis at bank.”  
- CC file becomes operational detail, not float.

**Option C — Link CC charges to one settlement**  
- Heavier: group many CC expenses under one bank settlement; settlement verifies the group.  
- Best long-term; more build cost.

**Recommendation for discussion:** start with **Option A** (exclude settlement lines from expense net / treat as transfer), document detection rules with the client (sample bank descriptions), leave Option C as phase 2.

Also: when reconciling, a bank CC settlement should **not** require a duplicate merchant expense match; it needs its own handling (“CC payment — exclude from net / link to CC period”).

---

## 12. Opening bank amount — product detail
...
| Updated when | A bank reconcile session is **completed successfully** (Gap ≈ 0 and no remaining unmatched requiring action — exact rule in §17) |
...
## 12b. High-priority alerts for incomplete verification
...
| Unmatched **app** transaction(s) in period | “App transaction(s) not in bank” — count + link | Matched, corrected, or explicitly excepted |
...
2. **Bank reconcile** (new page or strong mode inside Transactions upload)  
   - Upload bank Excel  
   - Match review (success path attaches bank אסמכתא; rows identified by transaction_ref)  
   - Gap = Bank balance − (Opening + Net)  
   - Opening balance + last verification date  
   - Completing with leftovers → auto high-priority alerts  

3. **Dashboard (v1)**  
   - **Last verification date** (required)  
   - Unverified count since that date  
   - Open bank-reconcile alert count / Gap hint  

4. **Alerts**  
   - Auto high-priority items for unmatched bank / unmatched app / Gap ≠ 0  
   - Filterable like other alerts; deep-link to reconcile  

---

## 14. Data model sketch (implementation later)
...
- `deposits` / `expenses`:  
  ...
  - optional `bank_reconcile_exclude`  
...
- Alert keys for reconcile leftovers (e.g. `bank_reconcile:unmatched_bank:{session_id}`, `…:unmatched_app:…`, `…:gap:…`) so they clear when the session is fixed  
- Optional: store raw bank lines so she can reopen a session  
...
## 15. Phased delivery (proposed)

| Phase | Scope | Outcome |
|-------|--------|---------|
| **P0** | Opening bank amount + **last verification date** on Dashboard; go-live cutover (mark baseline verified); **transaction_ref**; read bank balance / Gap | She can start production verified and see “verified through …” |
| **P1** | Bank upload as match/verify; Verified badge; **attach bank אסמכתא**; unmatched lists; **advance last verification date** on clean complete | Weekly verification → verification loop works |
| **P2** | **High-priority auto alerts** for unmatched bank / unmatched app / Gap ≠ 0 | She is pushed to fix leftovers |
| **P3** | CC settlement exclusion rules | Gap no longer broken by CC |
| **P4** | Auto-confirm high-confidence matches; polish | Less clicking |
| **P5** | Split one bank line → many txs; richer CC linking | Edge cases |
...
## 17. Decisions needed from bosses / client

Please confirm or rewrite:

1. **Equation:** Is `Gap = BankBalance − (Opening + CompanyFloatNet)` correct?  
2. **Net scope:** Verified-only vs all float rows for the success check?  
3. **Account scope:** Company operating account only for v1?  
4. **CC strategy:** Option A, B, or C for v1?  
...
7. **Bank balance (B):** Use file header **היתרה** (e.g. 114,834.88 as of print date), **first/latest row’s היתרה בש״ח**, or balance on a chosen date?  
...
10. **Bank אסמכתא:** Confirm every verified transaction must store the bank אסמכתא. Confirm composite fingerprint when bank אסמכתא repeats.  
...
12. **CC detection:** Confirm bank descriptions like `לאומי מאסטרקרד` are the settlement lines to exclude/special-case.  
13. **Alerts:** Confirm severity = error/high for unmatched bank, unmatched app, and Gap ≠ 0; confirm dismiss rules.  

---

## 18. Success criteria (after build)
...
- Each transaction has a **unique SimplifAI ref**; verified rows also show **bank אסמכתא**.  
- Missing pieces on either side (app or bank) create **high-priority alerts** automatically until resolved.  
- After a good week: Gap ≈ 0 and unmatched lists empty (or explicitly excepted).  
- CC settlements do not inflate net vs bank.  

---

## 19. Appendix A — Excel file structures (inspected)

Source folder: `data/ClientData/`.

### 19.1 `Bank Account example.xlsx` (Leumi account movements)
...
**Implication for Gap / opening balance:** the file exposes (1) header snapshot **היתרה** and (2) per-row **היתרה בש״ח**. Design must pick which drives **B** (§17.7). Opening amount **O** remains required so Gap can start at 0 when history predates SimplifAI.

### 19.2 `credit card 1 example.xlsx` (Leumi Mastercard detail)
...
### 19.3 What we already parse today
...
## 20. Appendix B — related code today (for engineers)
...
**Next step after approval:** turn §15 phases into tickets with UX mock acceptance criteria, then implement P0 → P1.
